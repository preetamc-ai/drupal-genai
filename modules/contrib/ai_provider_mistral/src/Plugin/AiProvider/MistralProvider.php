...
class MistralProvider extends AiProviderClientBase implements
  ...
  public function errorCatcher($errno, $errstr, $file, $line) {
    // Ignore deprecation warnings (E_USER_DEPRECATED) during our API calls.
    // We want to catch errors related to array_map error when the array keys
    // of responses differ.
    if ($errno === E_USER_DEPRECATED || $errno === E_DEPRECATED) {
      return TRUE;
    }
    else {
      throw new AiResponseErrorException("Something undefined was broken in the response from Mistral AI: $errstr at $file, line $line");
    }
  }

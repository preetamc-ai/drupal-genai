...
class PineconeProvider extends AiVdbProviderClientBase implements ContainerFactoryPluginInterface, DependentPluginInterface {
  ...
  private function processConditionGroup($index, ConditionGroupInterface $condition_group): array {
    ...
    foreach ($condition_group->getConditions() as $condition) {
      ...
      if ($condition instanceof ConditionGroupInterface) {
        ...
        // Use array_values() to strip string keys (e.g. "topic" => "topic")
        // and ensure we pass a flat indexed list to Pinecone.
        $values = is_array($condition->getValue()) ? array_values($condition->getValue()) : [$condition->getValue()];
        $filter = [];
  ...
  public function vectorSearch(
    ...
    if (in_array('values', $output_fields, TRUE)) {
      $include_values = TRUE;
    }

    $include_metadata = FALSE;
    if (
      (in_array('values', $output_fields, TRUE) && count($output_fields) > 1)
      || count($output_fields) > 0
    ) {
    ...
    foreach ($matches as $match) {
      ...
      // Build up additional data to pass back as normalized results.
      $add_data = [
        'distance' => $match['score'],
        'id' => $match['id'],
      ];
      if (isset($match['values'])) {
        $add_data['values'] = $match['values'];
      }

      $results[] = $match['metadata'] + $add_data;
    }
    return $results;
  }
  ...
  public function calculateDependencies() {
    return [
      'config' => [
        'ai_vdb_provider_pinecone.settings',
      ],
    ];
  }
